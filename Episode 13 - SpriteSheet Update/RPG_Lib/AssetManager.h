...
public:
	...
	bool LoadAsset(std::string assetLocation)
	{
		...
		{
			delete asset;
			printf("AssetManager:LoadAsset - Failed to load in asset: %s \n", assetLocation.c_str());
			return true;
		}
	...
	T* GetAsset(std::string assetLocation)
	{
		...
		if (a == m_Assets.end())
		{
			printf("AssetManager:LoadAsset - Failed to find asset: %s \n", assetLocation.c_str());
			return nullptr;
		}
		else
		{
			...
			{
				printf("AssetManager:LoadAsset - Failed to cast asset to: %s \n", typeid(T).name());
				return nullptr;
			}
		}
	}
