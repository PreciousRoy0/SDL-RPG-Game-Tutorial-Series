...
void IUIControl::Update(KeyboardState & keystate, MouseState & mouseState)
{
	...
	if (!UIrect.Contains(mouseState.Location) && m_MouseOver)
	{
		...
		if (m_MouseDown)
		{
			...
			mevent->State = mouseState;

			printf("KeyDown Click Cancel\n");

			OnMouseUp.SetEvent(this, mevent);
		}
	}

	//Mouse Down (1 check if ther is a change, 2 make shhure the change is down)
	...
	//Mouse up (1 check if ther is a change, 2 make shhure the change is released)
